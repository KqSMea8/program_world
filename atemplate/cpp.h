...
public:
    ...
    virtual double net_price(size_t n) const   //杩斿洖缁欏畾鏁伴噺鐨勪功绫嶇殑閿€鍞€婚锛屾淳鐢熺被鏀瑰啓骞朵娇鐢ㄤ笉鍚岀殑鎶樻墸璁＄畻鏂规硶
    {
        return n * price;
    }
...
private:
    string bookNo;             //涔︾睄鐨処SBN缂栧彿
protected:
    double price = 0.0;               //浠ｈ〃鏅€氱姸鎬佷笅涓嶆墦鎶樼殑浠锋牸
};
...
{
public:
    StrVec() : // allocator鎴愬憳杩涜榛樿鍒濆鍖?
        elements(nullptr), first_free(nullptr), cap(nullptr) {}
    StrVec(const StrVec&);  // 鎷疯礉鏋勯€犲嚱鏁?
    StrVec &operator=(const StrVec&);  // 璧嬪€艰繍绠楃
    ~StrVec();  // 鏋愭瀯鍑芥暟
    void push_back(const std::string&);  // 鎷疯礉鍏冪礌
...
private:
    std::allocator<std::string> alloc;  // 鍒嗛厤鍏冪礌
    // 琚坊鍔犲厓绱犵殑鍑芥暟鎵€浣跨敤
    void chk_n_alloc() { if (size() == capacity()) reallocate(); }
    // 宸ュ叿鍑芥暟锛岃鎷疯礉鏋勯€犲嚱鏁般€佽祴鍊艰繍绠楃鍜屾瀽鏋勫嚱鏁颁娇鐢?
    std::pair<std::string*, std::string*> alloc_n_copy(const std::string*, const std::string*);
    void free();  // 閿€姣佸厓绱犲苟閲婃斁鍐呭瓨
    void reallocate();  // 鑾峰緱鏇村鍐呭瓨骞舵嫹璐濆凡鏈夊厓绱?
    std::string *elements;  // 鎸囧悜鏁扮粍棣栧厓绱犵殑鎸囬拡
    std::string *first_free;  // 鎸囧悜鏁扮粍绗竴涓┖闂插厓绱犵殑鎸囬拡
    std::string *cap;  // 鎸囧悜鏁扮粍灏惧悗浣嶇疆鐨勬寚閽?
};
...
{
public:
    // 鍥犱负union鍚湁涓€涓猻tring鎴愬憳锛屾墍浠oken蹇呴』瀹氫箟鎷疯礉鎺у埗鎴愬憳
    // 瀹氫箟绉诲姩鏋勯€犲嚱鏁板拰绉诲姩璧嬪€艰繍绠楃鐨勪换鍔＄暀寰呮湰鑺傜粌涔犲畬鎴?
    Token() : tok(INT), ival(0) {}
    Token(const Token &t) : tok(t.tok) { copyUnion(t); }
    Token &operator=(const Token&);
    // 濡傛灉union鍚湁涓€涓猄ales_data鎴愬憳锛屽垯鎴戜滑蹇呴』閿€姣佸畠
...
private:
    enum {INT, CHAR, DBL, SDATA} tok;  // 鍒ゅ埆寮?
    union  // 鍖垮悕鑱斿悎
    {
        char cval;
        int ival;
        double dval;
        Sales_data sval;
    };  // 姣忎釜Token瀵硅薄鍚湁涓€涓鍖垮悕鑱斿悎绫诲瀷鐨勬湭鍛藉悕鎴愬憳
    void copyUnion(const Token&);  // 妫€鏌ュ垽鍒紡锛岀劧鍚庨厡鎯呮嫹璐漸nion鎴愬憳
};
...
typedef std::tuple<std::string, std::shared_ptr<std::set<line_no>>,
              std::shared_ptr<std::vector<std::string>>> QueryResult; // declaration needed for return type in the query function
class TextQuery {
public:
  TextQuery(std::ifstream&);
  QueryResult query(const std::string&) const;
